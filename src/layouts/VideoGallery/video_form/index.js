...
import { createVideoGallery, updateVideoGallery } from "Services/endpointes";
...
function VideoGalleryForm() {
  const form = useRef(null);
  const { id } = useParams();
  const isNewVideoGallery = id === "new-videogallery";
  ...
  const videoGalleryData = isNewVideoGallery ? {} : videogallery.find((e) => e.id === id) || {};
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      let data;
      if (isNewVideoGallery) {
        response = await ApiClient.createData(createVideoGallery, formData);
        setDialog(dispatch, [response.data]);
      } else {
        formData.append("id", id);
        response = await ApiClient.putData(updateVideoGallery, formData);
        console.log(response, "jytrdtyhhthtyryrhrhtyrhyrhrtytyrytyrtyrytrty");
  ...
  return (
    <DashboardLayout>
      <DashboardNavbar endpoint={"createvideogallery"} />
          ...
          <Card style={{ width: "100%" }}>
            ...
            >
              <SoftBox
                ...
                alignItems="center"
              >
                <h3>{isNewVideoGallery ? "Add Video-Gallery" : "Edit Video-Gallery"}</h3>
                ...
                <SoftBox mt={4} mb={1} width="18%" display="flex" justifyContent="space-between">
                  <SoftButton variant="gradient" color="warning" mr={2} onClick={handleSubmit}>
                    {isNewVideoGallery ? "Create" : "Update"}
                  </SoftButton>
                ...
                >
                  <SoftBox mb={2} width="100%">
                    ...
                    <SoftInput
                      ...
                      placeholder="Title"
                      fullWidth
                      defaultValue={videoGalleryData.title || ""}
                  ...
                  <SoftBox mb={2} width="100%">
                    ...
                    <SoftInput
                      name="src"
                      placeholder="iframe of youtube..."
                      defaultValue={videoGalleryData.src || ""}
                      minRows={4}
                      style={{
                        width: "100%",
                        padding: "10px",
